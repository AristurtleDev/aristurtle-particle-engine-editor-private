...
public sealed class ParticleEffectRendererNonStatic : IDisposable
{
    public Dictionary<string, Texture2D> Textures;
    public bool IsDisposed { get; private set; }

    public ParticleEffectRendererNonStatic()
    {
        Textures = new Dictionary<string, Texture2D>();
    }

    ~ParticleEffectRendererNonStatic() => Dispose(false);
    ...
    private unsafe void UnsafeDraw(SpriteBatch spriteBatch, ParticleEmitter emitter)
    {
        ...
        {
            throw new InvalidOperationException($"{nameof(ParticleEffectRendererNonStatic)} does not contain a texture named '{emitter.TextureKey}'.  Did you forget to add it?");
        }
